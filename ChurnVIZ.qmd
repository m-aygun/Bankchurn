...
![Customers churn](Churnpic.webp){fig-align="center"}
...
$Churn_Prob <- predict(fit_logit, newdata = test, type = "response")
...
bank_churn_fact$Log_Churn_Prob <- predict(fit_logit, newdata = bank_churn_fact, type = "response")
...
bank_churn_fact$LDA_Churn_Prob <- predict(lda.fit, newdata = bank_churn_fact)$posterior[, "Yes"]
...
for (i in seq_along(thresh)) {
  ...
  # Beregn confusion matrix og derefter omkostningerne
  ...
  # Her bruger vi en omkostningsformel, du må tilpasse den til din specifikke situation
  omkostninger_i <- FN * FN_omk + TP * TP_omk + FP * FP_omk + TN * 0
  
  # Tilføj til results_lda
  results_lda <- rbind(results_lda, data.frame(threshold = thresh[i], cost = omkostninger_i))
}

# Find det threshold med den laveste omkostning
optimal_threshold_lda <- results_lda[which.min(results_lda$cost), "threshold"]
optimal_cost_lda <- min(results_lda$cost)

print(paste("Optimalt threshold:", optimal_threshold_lda))
print(paste("Omkostninger ved optimalt threshold:", optimal_cost_lda))
...
bank_churn_fact$QDA_Churn_Prob <- predict(qda.fit, newdata = bank_churn_fact)$posterior[, "Yes"]
...
We can now asses the performance of the models. As we can se we are able to save money by focusing on datadriven decisions.
